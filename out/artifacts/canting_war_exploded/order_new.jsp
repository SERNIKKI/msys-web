                                    ...
                                    <div class="table-responsive order-table">
                                        <table class="table table-hover mb-0">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col">状态</th>
                                                <button  type="button" class="btn btn-primary" onclick="a(0)">
                                                    刷新
                                                </button>
                                                <th>
                                                    <div class="btn-group" role="group">
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1">
                                                            <a class="dropdown-item" onclick="a(0)">全部订单</a>
                                                            <a class="dropdown-item" onclick="a(1)">待通知</a>
                                                            <a class="dropdown-item" onclick="a(2)">待取餐</a>
                                                        </div>
                                                    </div>
                                                </th>
                                            </tr>
...
<%--                                详情列表--%>
    ...
    <script type="text/javascript">
                    ...
                    if(data.length<=5) {
                        for (i = 0; i < data.length; i++) {
                            html += '<tr>\n' +
                                ...
                                '                                        <td>' + data[i].date + '</td>\n';
                            if (data[i].state == 0) {
                                html += '                                         <td><span class="badge btn-primary badge-pill"><i class="mdi mdi-checkbox-blank-circle text-warning"></i>已下单</span></td>\n' +
                                    '                                        <td><button type="button" class="btn btn-primary order-ok waves-effect waves-light" id=' + data[i].uname + ' name=' + data[i].oid + ' onclick="inform(this,1)">通知取餐</button></td>\n' +
                                    '                                    </tr>';
                            } else {
                                html += '                                         <td><span class="badge badge-info badge-pill"><i class="mdi mdi-checkbox-blank-circle text-warning"></i> 待取餐</span></td>\n' +
                                    '                                        <td><button type="button" class="btn btn-info order-ok waves-effect waves-light" disabled="disabled">已通知</button></td>\n' +
                                    '                                    </tr>';
                            }
                        }
                    }else{
                ...
                success:function(data){
                    ...
                    if(data.length<=5*e){
                        for (i = 5*(e-1); i < data.length; i++) {
                            ...
                            if (data[i].state == 0) {
                                html += '                                         <td><span class="badge btn-primary badge-pill"><i class="mdi mdi-checkbox-blank-circle text-warning"></i>已下单</span></td>\n' +
                                    '                                        <td><button type="button" class="btn btn-primary order-ok waves-effect waves-light" id=' + data[i].uname + '  name=' + data[i].oid + ' onclick="inform(this,'+e+')">通知取餐</button></td>\n' +
                                    '                                    </tr>';
                            } else {
                                html += '                                         <td><span class="badge badge-info badge-pill"><i class="mdi mdi-checkbox-blank-circle text-warning"></i> 待取餐</span></td>\n' +
                                    '                                        <td><button type="button" class="btn btn-info order-ok waves-effect waves-light" disabled="disabled">已通知</button></td>\n' +
        ...
        function inform(o,e) {
            var id=o.name;
            var uname=o.id;
            var url="${pageContext.request.contextPath}/canting/servlet/order/inform"
            $.ajax({
                url:url,
                method:"post",
